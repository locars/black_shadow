...
@app.route('/livres/<string:mots>')
def search_livre(mots):
    mots = '%'+mots+'%'
    titre = livres.query.filter(livres.titre.like(mots)).all
...
def add_categorie():
    info = request.get_json()
    new_categorie = info['libelle_categorie']
    categorie = categorie(libelle_categorie=new_categorie)
    categorie.insert()
    return jsonify({
        'success' : True,
...
def ajoute_livres():
    info = request.get_json()
    isbn = info['code_ISBN']
    new_titre = info['titre']
    new_date = info['date_publication']
    new_auteur = info['auteur']
    new_editeur = info['editeur']
    categorie_id = info['categorie_id']
    livres = livres(isbn=isbn, titre=new_titre, date_publication=new_date, auteur=new_auteur, editeur=new_editeur, categorie_id=categorie_id)
    livres.insert()
    count = livres.query.count()
    return jsonify({
        'success' : True,
        'added' : livres.format(),
        'total.livres': count
...
@app.errerhandler(404)
def not_found(error):
    return jsonify({
        'success': False,
        'error' : 404,
        'message': "Recherche non disponible"
    }), 404
...
if __name__ == ' __main__':
    app.run(host= 'localhost', port=500 , debug = False)
